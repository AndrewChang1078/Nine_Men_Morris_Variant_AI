...
class board_game:
    ...
    def minimax_midgame_endgame(self, board, depth, color):
        ...
        if depth==0:
            ...
            for b in possible_board:
                temp=self.minimax_midgame_endgame(b, depth-1, 'B')
                if temp.est>ret.est:
                    ret.est=temp.est
                    ret.board=b
        elif color=='B':    #MINMAX
            ret.est=10000
            for b in possible_board:
                temp=self.minimax_midgame_endgame(b, depth-1, 'W')
                if temp.est<ret.est:
                    ret.est=temp.est
                    ret.board=b
        else:
...
if __name__ == "__main__":
    ...
    opt=g.minimax_midgame_endgame(g.board_position, int(depth), 'W')
