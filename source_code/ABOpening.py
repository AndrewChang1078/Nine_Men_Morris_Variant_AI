...
class board_game:
    ...
    def ab_opening(self, board, depth, color, alpha, beta):
        ...
        if depth==0:
            ...
            for b in possible_board:
                temp=self.ab_opening(b, depth-1, 'B', alpha, beta)
                if temp.est > ret.est:
                    ret.est=temp.est
                    ret.board=b
                if ret.est>=beta:
            ...
            ret.est=float('inf')
            for b in possible_board:
                temp=self.ab_opening(b, depth-1, 'W', alpha, beta)
                if temp.est < ret.est:
                    ret.est=temp.est
                    ret.board=b
                if ret.est<=alpha:
...
if __name__ == "__main__":
    ...
    opt=g.ab_opening(g.board_position, int(depth), 'W', -10000, 10000)
