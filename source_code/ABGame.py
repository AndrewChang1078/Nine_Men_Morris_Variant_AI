...
class board_game:
    ...
    def ab_midgame_endgame(self, board, depth, color, alpha, beta):
        ...
        if depth==0:
            ...
            for b in possible_board:
                temp=self.ab_midgame_endgame(b, depth-1, 'B', alpha, beta)
                if temp.est>ret.est:
                    ret.est=temp.est
                    ret.board=b
                if ret.est>=beta:
            ...
            ret.est=float('inf')
            for b in possible_board:
                temp=self.ab_midgame_endgame(b, depth-1, 'W', alpha, beta)
                if temp.est<ret.est:
                    ret.est=temp.est
                    ret.board=b
                if ret.est<=alpha:
...
if __name__ == "__main__":
    ...
    opt=g.ab_midgame_endgame(g.board_position, int(depth), 'W', -10000, 10000)
    open(output_file_name, 'w').close()   #Clear output file content
